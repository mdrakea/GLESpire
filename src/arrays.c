...
glopArrayElement(GLContext *c, GLParam *param)
{
  ...
  if (states & COLOR_ARRAY) {
    ...
    p[4].f = size > 3 ? c->color_array[i+3] : TGL_FIX_ONE;
    glopColor(c, p);  
  }
  ...
  if (states & TEXCOORD_ARRAY) {
    ...
    c->current_tex_coord.Z = size > 2 ? c->texcoord_array[i+2] : 0;
    c->current_tex_coord.W = size > 3 ? c->texcoord_array[i+3] : TGL_FIX_ONE;
  }
  if (states & VERTEX_ARRAY) {
    ...
    p[3].f = size > 2 ? c->vertex_array[i+2] : 0;
    p[4].f = size > 3 ? c->vertex_array[i+3] : TGL_FIX_ONE;
    glopVertex(c, p);
  }
}
